...
13. Memoization 자체에도 비용이 있다고 이해한다.
...
코드 복잡도
디버깅 난이도
잘못된 dependency로 인한 버그 가능성
```

그래서 컴포넌트나 함수가 보인다고 무조건 `React.memo`, `useCallback`, `useMemo`를 붙이는 것이 아니다. 작은 계산이라면 memoization을 관리하는 비용이 오히려 더 클 수 있다.

14. `useMemo`는 다음처럼 실제 계산 비용이나 참조 안정화의 의미가 있을 때 고려한다.

```text
큰 배열 filter / sort / map
복잡한 데이터 변환
차트 데이터 가공
비싼 계산
memo된 자식에게 전달하는 객체 / 배열 참조 안정화
```

반대로 문자열 결합처럼 매우 작은 계산을 무조건 `useMemo`로 감싸는 것은 의미가 거의 없을 수 있다.

15. `useCallback`은 다음처럼 **함수 참조 안정성 자체가 필요한 경우**에 고려한다.

```text
React.memo된 자식에게 함수 전달
다른 Hook의 dependency로 함수가 들어감
함수 참조 안정성이 실제로 필요함
```

단순한 버튼의 짧은 inline handler까지 무조건 `useCallback`으로 감쌀 필요는 없다.

16. `React.memo`는 특히 다음 조건이 겹칠 때 의미가 커질 수 있다고 이해한다.
...
큰 리스트 아이템, 복잡한 차트, 복잡한 폼 섹션 등이 후보가 될 수 있다. 반대로 렌더링 비용이 매우 작거나 props가 매번 바뀌는 컴포넌트에는 이득이 작을 수 있다.

17. Profiler에서는 단순히 "렌더링됐다"만 보는 것이 아니라 다음을 같이 본다.
...
그리고 왜 렌더링됐는지 추적할 때는 `state 변경`, `props 변경`, `parent render`, `context 변경`을 먼저 보고, 객체·배열·함수 props의 참조가 매번 새로 만들어지는지도 확인한다.
